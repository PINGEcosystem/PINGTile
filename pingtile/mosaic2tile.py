...
def doMosaic2tile(inFile: str,
    ...
    if total_win > 0:
        _diag = doMovWin(0, movWin.iloc[0], mosaic_reproj, target_size, outDir, outName, minArea_percent, windowSize, save_rgb=save_rgb, grayscale=grayscale)
        print(f'[mosaic2tile diag] window 0 result: {"accepted" if _diag is not None else "rejected"}')

    r = Parallel(n_jobs=threadCnt)(delayed(doMovWin)(i, movWin.iloc[i], mosaic_reproj, target_size, outDir, outName, minArea_percent, windowSize, save_rgb=save_rgb, grayscale=grayscale) for i in tqdm(range(total_win)))
